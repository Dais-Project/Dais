...
async def _load_task_runtime_context(task_id: int, agent_id: int) -> task_runtime_schemas.TaskRuntimeContext:
...
async def _load_schedule_runtime_context(task_id: int, agent_id: int) -> task_runtime_schemas.TaskRuntimeContext:
    async with db_context() as db_session:
...
async def load_task_runtime_context(
    ...
    match task_type:
        case task_runtime_schemas.TaskType.TASK:
            return await _load_task_runtime_context(task_id, agent_id)

...
- MCP 支持：支持接入更多外部工具，让 Agent 能处理更多任务场景
- Skills 管理：支持统一管理和分配 Agent 技能，让不同任务使用合适的能力组合
- 工作区管理：支持在多个工作区之间自由切换，适配不同项目和使用场景
- 多任务并行：支持同时处理和管理多个任务，让多任务处理更高效
- 多 Agent 协作：支持通过 subtask 来派发任务，高效利用上下文

...
from src.schemas import tasks as task_schemas 
...
class ScheduleJob:
    def __init__(self, schedule: task_schemas.ScheduleRead):
        self._schedule = schedule

    def _create_runtime_context(self, record: task_schemas.RunRecordRead) -> task_schemas.TaskRuntimeContext:
        return task_schemas.TaskRuntimeContext(
            id=record.id,
            usage=record.usage,
    ...
    async def __call__(self):
        async with db_context() as db_session:
            create = task_schemas.RunRecordCreate(schedule_id=self._schedule.id)
            created_record = await RunRecordService(db_session).create_run_record(create)

        runtime_context = self._create_runtime_context(task_schemas.RunRecordRead.model_validate(created_record))
...
class ScheduleRunner:
    ...
    async def append(self, id: int):
        ...
        job = ScheduleJob(task_schemas.ScheduleRead.model_validate(schedule))

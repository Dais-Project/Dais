...
export function ListDirectory({ message }: ToolMessageProps) {
  ...
  const toolArguments = useToolArgument<FileSystemListDirectory>(
    message,
  ...
  const content = (() => {
    if (message.isStreaming) {
      return (
        <p className="px-4 pb-4 text-muted-foreground text-sm">
          {t("tool.list_directory.generating")}
        </p>
      );
    }
    if (message.error) {
    ...
    if (toolArguments === null) {
      return (
        <p className="px-4 pb-4 text-muted-foreground text-sm">
          {t("tool.list_directory.parse_error")}
        </p>
      );
    }
  })();

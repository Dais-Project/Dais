...
from dais_sdk.types import TextChunkEvent, UsageChunkEvent, ToolCallChunkEvent
...
def agent_event_format(task: AgentTask, event: AgentEvent) -> ServerSentEvent | None:
    match event:
        ...
        case MessageChunkEvent(chunk):
            match chunk:
                case TextChunkEvent(content):
                    return JSONServerSentEvent(event=event.event_id, data={
                        "type": "text",
                        "content": content,
                    })
                case UsageChunkEvent() as chunk:
                    return JSONServerSentEvent(event=event.event_id, data={
                        "type": "usage",
                        "max_tokens": task._ctx.model.context_size,
                        **asdict(chunk),
                    })
                case ToolCallChunkEvent() as chunk:
                    return JSONServerSentEvent(event=event.event_id, data={
                        "type": "tool_call",
                        "data": asdict(chunk),
                    })
        ...
        case ToolExecutedEvent(call_id, result):
            return JSONServerSentEvent(event=event.event_id, data={
                "call_id": call_id,
                "result": result,
            })
        ...
        case ToolRequirePermissionEvent(call_id):
            return JSONServerSentEvent(event=event.event_id, data={
                "call_id": call_id,
            })
